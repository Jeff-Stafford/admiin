...
export function ClientDetail({
  ...
  const { data, error } = useQuery(gql(TASKS_BY_ENTITY_BY_CONTACT_ID), {
    variables: {
      entityIdBy: entityId,
      status: TaskSearchStatus.INCOMPLETE,
      contactId: contact?.id,
    },
    skip: !entityId || !contact?.id,
  });
    ...
    if (contact) {
      try {
        await updateContact({
          ...
          refetchQueries: [gql(CONTACTS_BY_ENTITY)],
        });
